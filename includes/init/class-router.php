...
use Instagram_Sales\Includes\PageHandlers\Product_Page_Handler;
 ...
 * @author     Your_Name <youremail@nomail.com>
 * @see        \Instagram_Sales\Includes\PageHandlers\Order_Page_Handler
 * @see        \Instagram_Sales\Includes\PageHandlers\Product_Page_Handler
 */
class Router {
    ...
    private function set_default_routes() {
        $this->routes = [
            '/products/show_product'        => Product_Page_Handler::class,
            '/order/status' => Order_Page_Handler::class,
        ];
    }
	...
	public function boot() {
		...
		if ( in_array( $current_route, $this->get_routes_keys(), true ) ) {
			...
			$handler_instance->render();//render of page handler
		}
	}
